...
public class VotingGroup implements AutoCloseable {
  ...
  private void addClientVoterNode(ClientVoterManager mgr, Properties connectionProps) {
    ...
    try {
      ...
      nodes.remove(mgr.getTargetHostPort());
      LOGGER.info("Unable to register with target {}", mgr.getTargetHostPort(), t);
    }
  }
  ...
  private String[] pollTopology(String[] existingTopo, Function<String, ClientVoterManager> factory, Properties connectionProps) {
    ...
    while (active != null) {
      try {
        ...
        Set<String> newTopology = active.getTopology();
        ...
        completePollingFuture(null);
      } catch (TimeoutException | RuntimeException e) {
        completePollingFuture(e);
      }
  ...
  private synchronized CompletableFuture<?> completePollingFuture(Exception e) {
    if (e != null) {
      pollingFuture.completeExceptionally(e);
    } else {
      pollingFuture.complete(null);
    }
    pollingFuture = new CompletableFuture<>();
    return pollingFuture;
  }
  
  public synchronized CompletableFuture<?> forceTopologyUpdate() {
    notifySleepTimer();
    return pollingFuture;
  }
  ...
  private VoterStatus getStatus() {
    return new VoterStatus() {
      ...
      public void awaitRegistrationWithAll() throws InterruptedException {
        CompletableFuture.allOf(bootstrapped, CompletableFuture.allOf(nodes.values().stream().map(ClientVoterThread::operational).toArray(CompletableFuture[]::new))).join();
      }

      @Override
      public void awaitRegistrationWithAll(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
        try {
          CompletableFuture.allOf(bootstrapped, CompletableFuture.allOf(nodes.values().stream().map(ClientVoterThread::operational).toArray(CompletableFuture[]::new))).get(timeout, unit);
